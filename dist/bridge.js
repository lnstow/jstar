...
class VM {
    ...
    static genericErrorHint(error, msg = VM.errorMsg) {
        ...
        if (error instanceof DOMException && error.name === "AbortError")
            ...
            VM.newHint("网络错误或跨域访问被禁止，要允许跨域访问，请查看帮助文档", ["去看看", Remote.gotoProject], ["还是算了", VM.emptyFunc]);
        else
            VM.newHint(msg);
        return false;
    }
    static async loadData() {
        ...
        if (orderMap.length == 0) {
            await VM.insertList(new NormalList("点我查看列表简介", [], "如何使用？请查看下面三个元素的说明"), 0);
            await VM.insertItem(new NormalItem("itemA", 2, "点我输入附带信息\n点击右下角按钮保存\n点击弹窗外部可以取消修改"), 0);
            await VM.insertItem(new NormalItem("itemB", 1, "整个网页的右下角有菜单图标，里面点击“帮助”查看如何多端同步数据"), 0, 1);
            await VM.insertItem(new NormalItem("itemC", 0, "长按元素可排序，编辑模式可以删除列表，点每行第二个图标新建列表"), 0, 2);
            vue.uis.show = true;
        }
        return VM.VUE_DATA;
    }
    ...
    static async updateItem(item, row, col, time = true) {
        if (!VM.checkUpdateTime())
            return false;
        if (time)
            item.date = getTime();
        else if (!VM.itemRef.has(item.sid))
            return false;
        try {
            await Repo.saveToDB(ITEM_TABLE, item);
