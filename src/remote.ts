    ...
    private static parserArr: ItemParser<Item>[] = [
        {
            ...
            async tryParse(html, temp: JaDb) {
                ...
                i1 = html.indexOf('ysi')
                if (i1 == -1) return await this.parseCorsVideo(temp)
            ...
            async parseCorsVideo(temp: JaDb): Promise<true> {
                const html = await Remote.fetch(
                    `https://www.r18.com/common/search/searchword=${temp.sid}/`
                )
                if (html == "") return true
                let i1 = 0, i2 = 0
